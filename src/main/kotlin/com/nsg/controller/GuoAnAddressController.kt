...
@Api(basePath = "/v1/mall/guoan", value = "guoanaddress", description = "国安用户地址API", produces = "application/json", position = 6)
@RestController
@RequestMapping("/v1/mall/guoan")
class GuoAnAddressController
@Autowired constructor(var addressService: AddressService) {
    ...
    fun addAddress(@RequestParam(value = "user_id", required = true) user_id: Long,
        ...
        try {
            ...
            if (isDefault) {
                addressService.setAddressDefault(mapOf("userId" to user_id, "addressId" to address.addressId))
            }

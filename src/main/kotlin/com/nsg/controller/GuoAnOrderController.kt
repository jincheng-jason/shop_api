...
@Api(basePath = "/v1/mall/guoan", value = "guoanorder", description = "国安订单API", produces = "application/json", position = 8)
@RestController
@RequestMapping("/v1/mall/guoan")
class GuoAnOrderController
@Autowired constructor(var orderService: OrderService,
    ...
    private val LOGGER = LoggerFactory.getLogger(GuoAnOrderController::class.java)
    ...
    fun addOrder(
        ...
        try {
            ...
            val uuid = orderService.getGuoanNhid(user_id)
            ...
            //将订单信息提交给支付中心
            var url = "${UrlEnum.PAYMENT_CENTER_URL.url}/pay/unifiedorder?unionuserid=${uuid}&out_trade_no=${orderSN}&total_fee=${orderForm.totalPrice}&body=${description}&appid=3";
            LOGGER.info(url)
...
//            }
    ...
    fun payOrder(@PathVariable order_id: Long,
        ...
        try {
            ...
            when (type) {
                PayType.WECHAT.type -> url = "${UrlEnum.PAYMENT_CENTER_URL.url}/wxpay/unifiedorder?out_trade_no=$orderSN&total_fee=$totalPrice&body=$body&period=1440&appid=3"
                PayType.ALIPAY.type -> url = "${UrlEnum.PAYMENT_CENTER_URL.url}/alipay/unifiedorder?out_trade_no=$orderSN&total_fee=$totalPrice&body=$body&period=1440&appid=3"
                else -> {
